...
import { CheckCircle2, XCircle, FileText, UserCheck, Mail, BookOpen, Loader2 } from "lucide-react";
    ...
    <PortalShell role="staff">
      <div className="space-y-6 max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div><h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Pending Student Verifications</h1><p className="text-muted-foreground text-sm mt-1">Review registrations and approve verified applications.</p></div>
            ...
            {registrations.map((reg) => (
              <Card key={reg.id} className="shadow-sm border">
                ...
                <CardContent className="space-y-4 text-sm">
                  ...
                  {reg.proofOfPaymentName && <div className="flex items-center gap-2 p-2.5 rounded-md border bg-background text-xs"><FileText className="h-4 w-4 text-primary shrink-0" /><span className="truncate font-medium flex-1">{reg.proofOfPaymentName}</span><Badge variant="outline" className="text-[10px] text-green-600">Uploaded</Badge></div>}
        ...
        <Dialog open={showApprovalModal} onOpenChange={setShowApprovalModal}>
          <DialogContent className="sm:max-w-md"><DialogHeader><div className="mx-auto w-12 h-12 rounded-full bg-green-100 text-green-600 flex items-center justify-center mb-2"><CheckCircle2 className="h-7 w-7" /></div><DialogTitle className="text-center text-xl">Student Approved Successfully!</DialogTitle><DialogDescription className="text-center">The Auth account and student portal record were created securely.</DialogDescription></DialogHeader>
            {approvedStudent && <div className="bg-muted/50 p-4 rounded-xl space-y-2.5 text-sm my-2 border"><div className="flex justify-between py-1 border-b"><span className="text-muted-foreground">Student Name:</span><span className="font-semibold">{approvedStudent.student.fullName}</span></div><div className="flex justify-between py-1 border-b"><span className="text-muted-foreground">Student Number:</span><Badge className="font-mono">{approvedStudent.studentNumber}</Badge></div><div className="flex justify-between py-1 border-b"><span className="text-muted-foreground">Email:</span><span>{approvedStudent.student.email}</span></div>{approvedStudent.password && <div className="flex justify-between py-1"><span className="text-muted-foreground">Temporary Password:</span><span className="font-mono font-bold">{approvedStudent.password}</span></div>}</div>}
